...
var import_later = __toESM(require("later"));
...
var Config = import_koishi.Schema.object({
  ...
  upcomingLimit: import_koishi.Schema.number().default(3).description("即将开始的活动显示数量")
  // 新增
});
async function getTodayEvents(webcalUrl, messageTemplate, logger, proxy, upcomingLimit = 3) {
  try {
...
function apply(ctx, config) {
  ...
  try {
    if (/at |every |on |after |before /i.test(config.cronTime)) {
      sched = import_later.default.parse.text(config.cronTime);
    } else {
      sched = import_later.default.parse.cron(config.cronTime, true);
    }
  ...
  scheduledTask = import_later.default.setInterval(async () => {
